...
if platform.system()=='Darwin':
    ...
    options.add_argument(r'--profile-directory=/Users/kisaki/Library/Application Support/Google/Chrome/Default')
    browser = webdriver.Chrome(options=options) # Chrome Driver Windows Path --if running on windows
...
#         browser.find_element(By.XPATH, "//input[@id='pie-store-finder-modal-search-field']").send_keys(zipcode) # Zip code
#     except:
#         browser.find_element(By.XPATH, "//input[@id='pie-store-finder-modal-search-field']").send_keys(input()) # Zip code
#     time.sleep(2.5) # lag for 3 seconds to allow elements to load
#     location = ', '.join(browser.find_element(By.CLASS_NAME, "wfm-search-bar--list_item")[0].text.split(', ')[-2:])
#     print('Getting items from the WholeFoods in ' + str(location) + '.')
#     browser.find_element(By.CLASS_NAME, "wfm-search-bar--list_item")[0].click()
# except Exception as e:
#     print(e)
...
try:
    load = browser.find_elements(By.XPATH,"//span[contains(text(),'Load more')]") 
    while True:
        load.click()
...
all_items = browser.find_elements(By.XPATH,"//div[@data-testid='product-tile']") # Pull all product elements by xpath
...
try:
    load = browser.find_element(By.XPATH,"//span[contains(text(),'Load more')]") 
    while True:
        load.click()
        time.sleep(2.5) # Must have a 2 sec time lag so the 'load more' button can reappear
except:
...
#############################################################################
print('There are ' + str(len(lifestyle) + len(produce) + len(dairy_eggs) + len(meat) + len(prepared_foods) + len(pantry_essentials) + len(bread_rolls_bakery) + len(desserts) + len(body_care) + len(supplements) + len(frozen_foods) + len(snacks_chips_salsas_dips) + len(seafood) + len(Beverages) + len(beauty) + len(floral)) + ' products on sale.')
#############################################################################
...
for category in list_of_categories:                                         #
    for i in range(len(globals()[category])):                               # At the range of the length of all items (will loop i times)
            if ('Add to list' in globals()[category][i][-1]) & (len(globals()[category][i]) >= 6):
                print(globals()[category][i])
            ...
            if ('Add to list' in globals()[category][i][-1]) & (len(globals()[category][i]) == 5):
